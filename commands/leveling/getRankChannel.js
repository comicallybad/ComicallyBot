...
module.exports = {
    ...
    run: (client, message, args) => {
        ...
        db.findOne({ guildID: guildID, channels: { $elemMatch: { command: "rank" } } }, (err, exists) => {
            ...
            else return message.reply(`Levelling's response channel is: <#${exists.channels[exists.channels.map(cmd => cmd.command).indexOf("rank")].channelID}>`).then(m => del(m, 7500));
