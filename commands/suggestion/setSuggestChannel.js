        ...
        function dbUpdate(channelID, channelName) {
            db.findOne({ guildID: guildID, channels: { $elemMatch: { command: "suggest" } } }, (err, exists) => {
                if (!exists) {
                    ...
                    return r(message.channel, message.author, "Suggest channel has been set!").then(m => del(m, 7500));
                } else {
                    ...
                    return r(message.channel, message.author, "Updated suggestion channel.").then(m => del(m, 7500));
                }
