    ...
    run: (client, message, args) => {
        ...
        function removeCoins(userID, coinsToRemove) {
            ...
            coins.findOne({ guildID: guildID, userID: userID }, (err, exists) => {
                if (!exists) return message.reply("This user has not yet been added to the database.").then(m => m.delete({ timeout: 7500 }))
                else {
                    if (exists.coins - coinsToRemove < 0) return message.reply("A user may not go below 0 coins").then(m => m.delete({ timeout: 7500 }))
                    else {
                        ...
                        return message.reply(coinsToRemove + " coins were removed to the user.").then(m => m.delete({ timeout: 7500 }));
                    }
                }
