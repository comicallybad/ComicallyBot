    ...
    run: (client, message, args) => {
        ...
        function getCoins(usrID) {
            coins.findOne({ guildID: guildID, userID: usrID }, (err, exists) => {
                if (!exists) return message.reply("User doesn't have coins yet.").then(m => m.delete({ timeout: 7500 }));
                if (exists.coins) return message.reply("User has: " + exists.coins + " coins").then(m => m.delete({ timeout: 7500 }));
                else return message.reply("User has no coins").then(m => m.delete({ timeout: 7500 }));
            })
        }
    }
}