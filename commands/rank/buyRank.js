    ...
    run: (client, message, args) => {
        ...
        if (args[0]) {
            if (ID) {
                if (message.member.roles.cache.find(r => r.id === ID)) {
                    return message.reply("You already have this role!").then(m => del(m, 7500));
                } else findRole(ID)
        ...
        function findRole(roleID) {
            ...
            db.findOne({ guildID: guildID, buyableRanks: { $elemMatch: { roleID: roleID } } }, (err, exists) => {
                ...
                } else return message.reply("This rank is not purchaseable").then(m => del(m, 7500));
            })
        }
        ...
        function purchase(cost, roleName, roleID) {
            ...
            coins.findOne({ guildID: guildID, userID: userID }, (err, exists) => {
                ...
                if (exists) {
                    ...
                    if (coinsHas <= cost) return message.reply("You do not have enough coins to purchase this rank!").then(m => del(m, 7500));
                    else {
                        author.roles.add(roleID)
                            .then(res => {
                                exists.coins -= cost
                                exists.save().catch(err => console.log(err));
                                message.reply(`You ave successfully purchased the ${roleName} role. Click your avatar to check it out!`).then(m => del(m, 7500));
