...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!toKick || !toKick.id) toKick = await client.users.fetch(args[0]);
        if (!toKick || !toKick.id) return r(message.channel, message.author, "Could not find that member!").then(m => del(m, 7500));
        ...
        try {
            ...
            if (err) return r(message.channel, message.author, `There was an error attempting to kick that member: ${err}`).then(m => del(m, 7500));
        }
    }
}