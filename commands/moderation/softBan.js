...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        try {
            ...
            if (!toBan) return r(message.channel, message.author, "Could not find that member!").then(m => del(m, 7500));
            ...
            if (err) return r(message.channel, message.author, `There was an error attempting to softban that member: ${err}`).then(m => del(m, 7500));
        }
    }
}