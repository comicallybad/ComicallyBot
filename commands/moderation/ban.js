...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        try {
            ...
            if (!toBan || !toBan.id) toBan = await client.users.fetch(args[0]);
            if (!toBan || !toBan.id) return r(message.channel, message.author, "Could not find that member!").then(m => del(m, 7500));
            ...
            return s(message.channel, '', promptEmbed).then(async msg => {
                ...
                if (emoji === "✅") {
                    ...
                    }).catch(err => r(message.channel, message.author, `There was an error attempting to ban ${toBan} ${err}`).then(m => del(m, 7500)));
                } else if (emoji === "❌") {
                    del(msg, 0);
            ...
            if (err) return r(message.channel, message.author, `There was an error attempting to ban that member: ${err}`).then(m => del(m, 7500));
        }
    }
}