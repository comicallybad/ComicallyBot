...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        }).then(res => s(message.channel, `Response from discordbotlist \`statusText: ${res.statusText}\``)
            .then(m => del(m, 7500)))
            .catch(err => s(message.channel, ` There was an error attempting to update discordbotlists stats. ${err}`));
        ...
        }).then(res => s(message.channel, `Response from top.gg \`statusText: ${res.statusText}\``)
            .then(m => del(m, 7500)))
            .catch(err => s(message.channel, `There was an error attempting to update top.gg stats. ${err}`));
