...
module.exports = {
    ...
    run: (client, message, args) => {
        ...
        function dbUpdate(channelID, channelName) {
            return db.findOne({ guildID: guildID, channels: { $elemMatch: { command: "verify" } } }, (err, exists) => {
                if (!exists) {
                    ...
                    return r(message.channel, message.author, "Updated verification channel.").then(m => del(m, 7500));
                }
            }).catch(err => err);
        }
    }
